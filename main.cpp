...
// Checks user input to see if they have inputted a number
...
int main()
{
    ...
    do
    {
        // displays players hand, coins and chains
        ...
        while ((input == 'y' || input == 'Y') && !table.p1.hand.hand.empty())
        {
            ...
            if (input == 'y' || input == 'Y')
            {
                ...
                for (int i = 0; i < table.p1.getMaxChains(); i++)
                {
                    ...
                    // if the chains empty we can add any card to the chain if it is empty
                ...
                // if the player choses to play thier top card they can play it if it matches, if it doesnt then they cant play
        ...
        //! PLAYER 2 TURN, very inefficent to code like this but a for loop with passinf by refrence wasnt working
        // player 2 turn the exact same as above
        if (table.deck->deck.empty())
        {
            checkWhoWon(table);
            break;
...
void checkWhoWon(Table t)
{
    ...
    {
        cout << p1 << " WINS!!!!" << endl;
        cout << t.p1.getName() << " coins: " << t.p1.getNumCoins() << " vs " << t.p2.getName() << " coins: " << t.p2.getNumCoins() << endl;
    }
    else
    {
